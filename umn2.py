...
def salvar_agendamento(nome, idade, sexo, tipo_doacao, tipo_sanguineo, data_hora, hora):
...
if "Ops" not in st.session_state:
    st.session_state.ops = [
        ...
        'Agendamento', 'Vídeo Informativo', 'Agradecimentos'
    ]
...
if pagina == "Elegibilidade de Doação":
    ...
    with st.form("form_agendamento"):
        ...
        if submit_agendamento:
            if not (nome):
                ...
                salvar_agendamento(nome, idade, sexo, tipo_doacao, tipo_sanguineo, data_hora, hora)
                st.success("Agendamento realizado com sucesso!")

            
    st.button('Ir para Vídeo Informativo', type="primary", on_click=MudarG)
    ...
    st.video("Vídeo.mp4")
    st.button('Ir para Agradecimentos', type="primary", on_click=MudarH)
